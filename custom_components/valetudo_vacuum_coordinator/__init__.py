...
COORDINATOR_SCHEMA = vol.Schema(
    {
        ...
        vol.Optional(
            CONF_NATIVE_RESUME_ENABLED,
            default=DEFAULT_NATIVE_RESUME_ENABLED,
        ): cv.boolean,
        vol.Optional(
            CONF_NATIVE_RESUME_TIMEOUT,
            default=DEFAULT_NATIVE_RESUME_TIMEOUT,
        ): cv.positive_int,
        vol.Optional(
            CONF_NAVIGATION_ERROR_RETURN_ENABLED,
            default=DEFAULT_NAVIGATION_ERROR_RETURN_ENABLED,
        ): cv.boolean,
        vol.Optional(
            CONF_DOCK_SETTLE,
            default=DEFAULT_DOCK_SETTLE,
        ): vol.All(vol.Coerce(int), vol.Range(min=0)),
        vol.Optional(
            CONF_DISPATCH_START_TIMEOUT,
            default=DEFAULT_DISPATCH_START_TIMEOUT,
        ): cv.positive_int,
        vol.Optional(
            CONF_BLOCKED_SESSION_TIMEOUT,
